...
def gain_neighborhood_band(x_train, band, band_patch, patch = 5):
    ...
    x_train_band = np.zeros((x_train.shape[0], patch * patch * band_patch, band)).astype(np.float16)
    # 中心区域
    ...
    for i in range(nn):
        if pp > 0:
            ...
            x_train_band[:, (nn + 1 + i):(nn + 2 + i), :(band - i - 1)] = x_train_reshape[:, 0:1, (i + 1):]
    x_train_band = np.transpose(x_train_band, (0, 2, 1))
    print("x_train_band  shape = {}, type = {}".format(x_train_band.shape, x_train_band.dtype))
    print("**************************************************")
    return x_train_band


def make_data(dataset: str, patch_size = 5, band_patch = 5, n_class = 2):
    ...
    pixel_patches_t1 = get_patches(mirror_t1, img_height, img_width, channel, patch_size).astype(np.float16)
    pixel_patches_t2 = get_patches(mirror_t2, img_height, img_width, channel, patch_size).astype(np.float16)

    # get band group
    # [img_height * img_width, band_size, band_patch * patch_size**2]
    x1_train_band = gain_neighborhood_band(pixel_patches_t1, channel, band_patch, patch_size).astype(np.float16)
    x2_train_band = gain_neighborhood_band(pixel_patches_t2, channel, band_patch, patch_size).astype(np.float16)
    ...
    x = np.concatenate((x1_train_band, x2_train_band), axis = 1)
    target = y
